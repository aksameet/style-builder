...
class EditBoxShadow extends React.Component {
  ...
  getShadow = () => {
    if(this.state.x && this.state.y && this.state.blur && this.state.color) {
      this.props.onChange(`${this.state.x} ${this.state.y} ${this.state.blur} ${this.state.color}`);
    } else {
      this.props.onClear();
    }
  };
  ...
  render() {
    return (
      <div className="sb-editTextShadow">
        <Colorpicker
